...
class Attention(nn.Module):
    ...
    def forward(self, x, policy):
        ...
        # print(attn.shape,attn[:, :, 0, :].unsqueeze(2).shape)
        # print('attn 0', attn[:, :, 0, :].unsqueeze(-1).mean(dim=1).mean(dim=1, keepdim=True), attn[:, :, 0, :].unsqueeze(-1).mean(dim=1).mean(dim=1, keepdim=True).shape)
        # print('compare', attn[:, :, 0, :].unsqueeze(-1).mean(dim=1) > attn[:, :, 0, :].unsqueeze(-1).mean(dim=1).mean(dim=1, keepdim=True), torch.nonzero(attn[:, :, 0, :].unsqueeze(-1).mean(dim=1) > attn[:, :, 0, :].unsqueeze(-1).mean(dim=1).mean(dim=1, keepdim=True)), attn[:, :, 0, :].unsqueeze(2).mean(dim=1, keepdim=True).shape ,attn[:, :, 0, :].unsqueeze(-1).mean(dim=1).shape, (attn[:, :, 0, :].unsqueeze(-1).mean(dim=1) > attn[:, :, 0, :].unsqueeze(-1).mean(dim=1).mean(dim=1, keepdim=True)).shape)
        # print(torch.count_nonzero(attn[:, :, 0, :].unsqueeze(-1).mean(dim=1) > attn[:, :, 0, :].unsqueeze(-1).mean(dim=1).mean(dim=1, keepdim=True)))
        if policy is None:
            attn = attn.softmax(dim=-1)
        else:
            attn = self.softmax_with_policy(attn, policy)
            #, torch.count_nonzero((attn[:, :, 0, :] > attn[:, :, 0, :].mean(dim=2).unsqueeze(-1))[:,0,:]),torch.count_nonzero((attn[:, :, 0, :] > attn[:, :, 0, :].mean(dim=2).unsqueeze(-1))[:,1,:]),torch.count_nonzero((attn[:, :, 0, :] > attn[:, :, 0, :].mean(dim=2).unsqueeze(-1))[:,2,:]),torch.count_nonzero((attn[:, :, 0, :] > attn[:, :, 0, :].mean(dim=2).unsqueeze(-1))[:,3,:]))
